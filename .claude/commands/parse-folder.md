---
description: Run captures-to-md on a folder to convert new PDFs/images/Office docs into markdown via the Extend Parse API. Use when the user wants to ingest, parse, or process files from a folder.
argument-hint: "<path to folder>"
allowed-tools: Bash(captures-to-md:*), Bash(find:*), Bash(ls:*), Read
---

The user wants to parse files from this folder: $ARGUMENTS
...
3. If `processed > 0`: list the newly-created `.md` files in `parsed_outputs/` (those modified in the last 15 minutes). Offer to summarize or open any of them.
